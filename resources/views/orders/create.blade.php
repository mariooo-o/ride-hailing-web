...
<div class="row justify-content-center">
    <div class="col-md-7">
        <div class="d-flex align-items-center mb-4 gap-3">
            ...
            <h4 class="fw-bold mb-0">Buat Order Baru</h4>
        </div>
        ...
        <div class="card">
            <div class="card-body p-4">
                <form method="POST" action="{{ route('orders.store') }}">
                    ...
                    <div class="mb-4">
                        <label class="form-label fw-semibold">Lokasi Pickup</label>
                        <div class="input-wrapper">
                            <input type="text" name="pickup" id="pickup"
                                ...
                                value="{{ old('pickup') }}" autocomplete="off" required>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="input-wrapper">
                            <input type="text" name="destination" id="destination"
                                ...
                                value="{{ old('destination') }}" autocomplete="off" required>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="vehicle_type" class="form-select @error('vehicle_type') is-invalid @enderror">
                            <option value="Motor" {{ old('vehicle_type') === 'Motor' ? 'selected' : '' }}>Motor - Rp 2.500/km (min. Rp 10.000)</option>
                            <option value="Mobil" {{ old('vehicle_type') === 'Mobil' ? 'selected' : '' }}>Mobil - Rp 5.000/km (min. Rp 20.000)</option>
                        </select>
                        @error('vehicle_type')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
...
<style>
    ...
    .suggestions-list li:hover { background-color: #f0f7ff; color: #0d6efd; }
    .suggestions-list .loading-item { color: #6c757d; font-style: italic; pointer-events: none; }
        ...
        delay = setTimeout(() => {
            ...
            fetch(url, { headers: { 'Accept-Language': 'id' } })
                ...
                .then(data => {
                    ...
                    if (data.length === 0) { suggestions.innerHTML = '<li class="loading-item">Lokasi tidak ditemukan</li>'; return; }
                    data.forEach(place => {
                        const li = document.createElement('li');
                        const name = place.display_name.length > 80 ? place.display_name.substring(0, 80) + '...' : place.display_name;
                        li.textContent = name;
                        li.title = place.display_name;
                        li.addEventListener('click', () => { input.value = place.display_name; suggestions.innerHTML = ''; });
                        suggestions.appendChild(li);
                    });
                })
                .catch(() => { suggestions.innerHTML = '<li class="loading-item">Gagal memuat saran lokasi</li>'; });
        }, 500);
    });
    document.addEventListener('click', e => { if (!input.contains(e.target) && !suggestions.contains(e.target)) suggestions.innerHTML = ''; });
    input.addEventListener('keydown', e => { if (e.key === 'Escape') suggestions.innerHTML = ''; });
}
